...
/** Preview releases open GitHub manually; signed stable builds may install in-app. */
export type DesktopUpdateMode = 'manual-release' | 'signed-auto'

/** Electron platforms supported by the DeepSeek Harness Desktop Gala native adapter. */
...
export interface DesktopUpdateAdapter {
  ...
  /** Security gate selected at build/runtime; unsigned Preview is always manual. */
  ...
  /** Open one validated public GitHub Release in the system browser. */
  openRelease(url: string): Promise<void>
  /** Ask whether one strictly newer signed version may be downloaded. */
  ...
  /** Seed electron-updater and verify it resolved the expected version. */
  prepareAutoUpdate(version: string): Promise<boolean>
  /** Download a signed update through electron-updater with cancellation. */
  downloadUpdate(version: string, signal: AbortSignal): Promise<void>
  /** Ask whether the downloaded signed update should restart now. */
  confirmInstall(version: string): Promise<boolean>
  /** Trigger electron-updater's verified quit-and-install flow. */
  quitAndInstall(): void
...
export interface DesktopRuntime {
  ...
  /** Native network, signed updater, and notification adapter. */
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    /** Electron adapter provided by the DeepSeek Harness Desktop Gala launcher. */

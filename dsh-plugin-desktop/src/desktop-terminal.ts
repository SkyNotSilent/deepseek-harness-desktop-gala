/** Isolated command-line environment launched from the DeepSeek Harness Desktop Gala tray. */
...
/** Platforms with a native terminal launch contract owned by DeepSeek Harness Desktop Gala. */
...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`DeepSeek Harness Desktop Gala ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart DeepSeek Harness Desktop Gala after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("DeepSeek Harness Desktop Gala {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart DeepSeek Harness Desktop Gala after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(DeepSeek Harness Desktop Gala !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart DeepSeek Harness Desktop Gala after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "DeepSeek Harness Desktop Gala" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}

...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode before loading the fixed packaged CLI entry', async () => {
    ...
    const argv = ['/Applications/DeepSeek Harness Desktop Gala', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/DeepSeek Harness Desktop Gala',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/DeepSeek Harness Desktop Gala.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/DeepSeek Harness Desktop Gala.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\DeepSeek Harness Desktop Gala\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\DeepSeek Harness Desktop Gala\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/DeepSeek Harness Desktop Gala.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/DeepSeek Harness Desktop Gala.app/Contents/Resources/app.asar.unpacked/package.json')
    expect(unpackedAsarPath('/workspace/node_modules/pkg')).toBe('/workspace/node_modules/pkg')
    expect(packagedDependencyPath(
      'file:///Applications/DeepSeek%20Harness%20Desktop%20Gala.app/Contents/Resources/app.asar/lib/desktop-cli.js',
      '@deepseek-ai/dsh/lib/bin.js',
    )).toBe(
      '/Applications/DeepSeek Harness Desktop Gala.app/Contents/Resources/app.asar.unpacked/node_modules/@deepseek-ai/dsh/lib/bin.js',
    )

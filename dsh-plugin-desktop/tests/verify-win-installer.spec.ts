...
function fixture(version = '2.0.0'): {
  ...
  const installer = join(dist, `DeepSeek-Harness-Desktop-Gala-${version}-x64-Setup.exe`)
  const application = join(unpacked, 'DeepSeek Harness Desktop Gala.exe')
...
describe('Windows installer artifact verification', () => {
  ...
  it('rejects a stale installer from a different version', () => {
    ...
    expect(() => verifyWindowsInstaller({ desktopRoot: value.root, version: '2.0.0' }))
      .toThrow('DeepSeek-Harness-Desktop-Gala-2.0.0-x64-Setup.exe')
  })

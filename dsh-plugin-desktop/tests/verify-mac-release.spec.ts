...
function options(overrides: Partial<MacReleaseVerificationOptions> = {}) {
  ...
  const value: MacReleaseVerificationOptions = {
    ...
    listDmgs: () => ['/release/dist/DeepSeek Harness Desktop Gala-2.0.0-arm64.dmg'],
...
describe('macOS release artifact verification', () => {
  it('mounts one DMG and verifies signature, Gatekeeper, and the stapled ticket', () => {
    ...
    expect(verifyMacRelease(harness.value)).toEqual({
      appPath: '/private/tmp/dsh-desktop-dmg-test/DeepSeek Harness Desktop Gala.app',
      dmgPath: '/release/dist/DeepSeek Harness Desktop Gala-2.0.0-arm64.dmg',
    })
    ...
    expect(harness.calls).toEqual([
      ...
      {
        command: 'spctl',
        args: ['--assess', '--type', 'execute', '--verbose=4', '/private/tmp/dsh-desktop-dmg-test/DeepSeek Harness Desktop Gala.app'],
      },
      {
        command: 'xcrun',
        args: ['stapler', 'validate', '/private/tmp/dsh-desktop-dmg-test/DeepSeek Harness Desktop Gala.app'],
      },
      {
        command: 'hdiutil',

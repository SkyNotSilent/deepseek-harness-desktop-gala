...
const updater = vi.hoisted(() => ({
  ...
  cancellations: [] as Array<{ cancel: ReturnType<typeof vi.fn> }>,
}))
...
vi.mock('electron-updater', () => ({
  default: { autoUpdater: updater.auto },
  autoUpdater: updater.auto,
  CancellationToken: class CancellationToken {
    readonly cancel = vi.fn()
    constructor() { updater.cancellations.push(this) }
  },
}))
...
describe('Electron compatibility runtime', () => {
  ...
  it('rebuilds ordered effect-scoped tray contributions without replacing native commands', async () => {
    ...
    expect(labels).toEqual([
      'Open DeepSeek Harness Desktop Gala', undefined,
  ...
  it('opens the active profile through the packaged terminal adapter', async () => {
    ...
    try {
      ...
      expect(terminal.open).toHaveBeenCalledWith(expect.objectContaining({
        ...
        profileName: 'desktop',
        productVersion: '2.1.0-preview.3',
        profileDir: '/tmp/dsh-home/profiles/desktop',
        homeDir: '/tmp/dsh-home',
  ...
  it('locks Preview builds to manual GitHub Releases and disables updater auto behavior', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
    const response = Response.json([])
    electron.net.fetch.mockResolvedValueOnce(response)
    const { ElectronDesktopRuntime, desktopProductMetadata, desktopUpdateMode } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})

    await expect(runtime.updates.request('https://api.github.com/repos/SkyNotSilent/deepseek-harness-desktop-gala/releases', { method: 'GET' }))
      .resolves.toBe(response)
    expect(runtime.updates).toMatchObject({
      isPackaged: false,
      mode: 'manual-release',
      currentVersion: '2.1.0-preview.3',
    ...
    expect(desktopUpdateMode(true, '2.1.0', 'signed-auto', 'darwin')).toBe('signed-auto')
    expect(desktopUpdateMode(true, '2.1.0', undefined, 'win32')).toBe('manual-release')
    expect(updater.auto.autoDownload).toBe(false)
    expect(updater.auto.autoInstallOnAppQuit).toBe(false)
    expect(updater.auto.allowPrerelease).toBe(false)
    expect(updater.auto.downloadUpdate).not.toHaveBeenCalled()
    expect(updater.auto.quitAndInstall).not.toHaveBeenCalled()
  })

  it('opens only the trusted public GitHub Release', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    const url = 'https://github.com/SkyNotSilent/deepseek-harness-desktop-gala/releases/tag/v2.1.0-preview.2'

    await runtime.updates.openRelease(url)
    expect(electron.shell.openExternal).toHaveBeenCalledWith(url)
    await expect(runtime.updates.openRelease('https://evil.example/update')).rejects.toThrow('untrusted')
    await expect(runtime.updates.openRelease('https://user:pass@github.com/SkyNotSilent/deepseek-harness-desktop-gala/releases/tag/v9.9.9'))
      .rejects.toThrow('untrusted')
  })

  it('reports manual check results and native notifications', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('darwin')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    ...
    await runtime.updates.showManualCheckResult({
      ...
      releaseUrl: 'https://github.com/SkyNotSilent/deepseek-harness-desktop-gala/releases/tag/v2.1.0-preview.1',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      ...
      detail: 'Installed version: 2.1.0-preview.1',
    }))
    ...
    runtime.updates.notify({ title: 'Profile Recovered', body: 'Reopened the profile.' })
    expect(electron.notifications[0]?.options).toEqual({
      title: 'Profile Recovered',
      body: 'Reopened the profile.',
    })
  })
